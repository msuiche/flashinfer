...
acc_stages = 2  # 16 acc cols/stage
...
epi_stage_rows = 128
epi_chunk_elems = 16
ab_stages = 16  # SMEM-D 8208B fixed + cast LOAD 0B/stage + multi-GEMM 0B/stage
        ...
        while is_valid != 0:
            ...
            for mi in cutlass.range_constexpr(mma_size_m):
                ...
                for subtile_idx in cutlass.range_constexpr(subtile_cnt):
                    ...
                    _op_0 = _c_0_a * (cutlass.full_like(_c_0_a, _aux_scale_pre.to(cutlass.Float32)))
...
def compile() -> Callable:
    ...
    return cute.compile(_host, problem_size, fake_first_token_offset, fake_tma_workspace, fake_a_0, fake_b_0, fake_scale, fake_c_0, stream=_fake_stream, options=frost_compile_options)
