...
# Tile config: CONFIG_sm100_128x64x128_128x64x32_cluster2x1_2ctamma_swapAB
mma_inst_shape_mnk = (256, 64, 16)
mma_k_dim = 0
cta_group = 2
cgrp_tile_mnk = (256, 64, 64)
cta_tile_mnk = (128, 32, 64)
epi_tile_mn = (128, 32)
threads_per_cta = 256
...
acc_stages = 2  # 128 acc cols/stage
grid_num_clusters = 108
...
epi_stage_rows = 128
epi_chunk_elems = 1
ab_stages = 8  # SMEM-D 0B fixed + cast LOAD 0B/stage + multi-GEMM 16384B/stage
...
@cute.kernel
def frost_sm100_moe_grouped_matmul_fwd_swap_ab_128x64x128_128x64x32_cluster2x1_2ctamma_swapAB_stg(
    m: cutlass.Int64,
    n: cutlass.Int64,
    k: cutlass.Int64,
                    ...
                    if row_active and row < M:
                        for j in cutlass.range_constexpr(subtile_w // vsize):
                            ...
                            if col_j < group_end:
                                ...
                                _r1 = _c_1_a
                                _rk1 = cute.math.max(_r1, cutlass.full_like(_r1, cutlass.Float32(0.0))) + cutlass.full_like(_r1, cutlass.Float32(1.0)) * cute.math.min(_r1, cutlass.full_like(_r1, cutlass.Float32(0.0)))
                                _ru1 = cute.math.min(_rk1, cutlass.full_like(_r1, cutlass.Float32(7.0)))
                                _op_1 = _ru1
                                _c_2_a = (vec_f32_1).to(cutlass.Float32)
                                _r2 = _c_2_a
                                _rk2 = cute.math.max(_r2, cutlass.full_like(_r2, cutlass.Float32(0.0))) + cutlass.full_like(_r2, cutlass.Float32(1.0)) * cute.math.min(_r2, cutlass.full_like(_r2, cutlass.Float32(0.0)))
                                _rl2 = cute.math.max(_rk2, cutlass.full_like(_r2, cutlass.Float32(-7.0)))
                                _ru2 = cute.math.min(_rl2, cutlass.full_like(_r2, cutlass.Float32(7.0)))
                                _op_2 = _ru2
                                _c_3_a = (_op_1).to(cutlass.Float32)
                                _c_3_b = (_op_2).to(cutlass.Float32)
                                _op_3 = _c_3_a * _c_3_b
                                _c_4_a = (_op_3).to(cutlass.Float32)
                                _op_4 = _c_4_a * (cutlass.full_like(_c_4_a, _aux_scale_pre.to(cutlass.Float32)))
                                _r_4 = (_op_4).to(cutlass.BFloat16)
                                _tap_0 = (_r_4).to(cutlass.BFloat16)
                                (gC_tap_0_ptr + row * out_stride_m_0 + (col_j + 0) * out_stride_n_0).store(_tap_0[0 : 0 + 1], alignment=2)
...
frost_sm100_moe_grouped_matmul_fwd_swap_ab_128x64x128_128x64x32_cluster2x1_2ctamma_swapAB_stg.set_name_prefix("cudnn", remove_cutlass_symbol=True)
...
def _host(
    ...
    frost_sm100_moe_grouped_matmul_fwd_swap_ab_128x64x128_128x64x32_cluster2x1_2ctamma_swapAB_stg(
        problem_size[0],
        problem_size[1],
        problem_size[2],
...
def compile() -> Callable:
    ...
    return cute.compile(_host, problem_size, fake_first_token_offset, fake_tma_workspace, fake_a_0, fake_a_1, fake_b_0, fake_c_tap_0, fake_scale, stream=_fake_stream, options=frost_compile_options)
