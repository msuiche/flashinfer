...
def _module(arch):
    from ...jit.core import gen_jit_spec, sm100a_nvcc_flags, sm107a_nvcc_flags

    flags = {"sm_100a": sm100a_nvcc_flags, "sm_107a": sm107a_nvcc_flags}[arch]
...
class CudnnFrostBf16MoeRunner(MoERunner):
    ...
    def _check_support(self):
        ...
        if self.device.type != "cuda":
            raise NotImplementedError(
                "cuDNN Frost BF16 MoE kernels require SM100a or SM107a"
            )
    ...
    def accepts(self, act, weights):
        ...
        try:
            ...
            return False
        first, second = _kernels(t * k, h, i, e, self.device, self.config.activation)
