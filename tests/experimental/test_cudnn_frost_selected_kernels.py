...
supported_gpu = pytest.mark.skipif(
    ...
    reason="packaged cuDNN Frost sources target SM100a and SM107a",
)
...
@pytest.mark.parametrize("arch", [100, 107])
def test_layer_adds_independent_candidate_and_separates_winner_cache(monkeypatch, arch):
    ...
    layer.config, layer.device, layer._arch = cfg, torch.device("cuda", 0), arch
...
def test_auto_admission_only_large_supported_geometry():
    ...
    assert not large_bf16_moe(cfg, act, 107)  # measured slower than original TRTLLM
...
def test_automatic_candidate_declines_missing_source_compiler(
    ...
    _assert_compiler_rejected("cannot import cutlass.experimental.primitives")
...
def test_compiler_admission_requires_native_arch(monkeypatch, compiler_probe):
    ...
    # Lack of native Rubin support must not disable the supported Blackwell pool.
    monkeypatch.setattr(moe, "get_compute_capability", lambda device: (10, 0))
    assert moe.automatic_candidate(config(), torch.device("cuda", 0)) is not None
...
@pytest.mark.parametrize("cc", [(10, 0), (10, 7)])
def test_compiler_admission_uses_capabilities_without_compiling(
    cc, monkeypatch, compiler_probe
):
    import cutlass.cute as cute
    ...
    monkeypatch.setitem(sys.modules, "cudnn", None)
    monkeypatch.setattr(moe, "get_compute_capability", lambda device: cc)
    device = torch.device("cuda", 0)
    assert moe.automatic_candidate(config(), device) is not None
