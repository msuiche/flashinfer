...
def benchmark(args):
    ...
    backend_configs = (CutlassBf16Config(),)
    if args.activation in ("swiglu", "relu2"):
        backend_configs += (TrtllmBf16Config(),)
    config = MoEConfig(
...
def export_task(opts):
    ...
    try:
        ...
        if opts.warm_source_jit:
            ...
            # Shape-independent generated bytes share a canonical research path,
            # so compiling the same source for another geometry hits the cache.
            source = opts.output_dir / kernel["source"]["path"]
            digest = kernel["source"]["sha256"]
            target = source.with_name(
                f"cudnn_frost_{opts.op}_{opts.config}_{opts.store_mode}_{digest[:16]}.py"
            )
            if target.exists():
                if hashlib.sha256(target.read_bytes()).hexdigest() != digest:
                    raise RuntimeError(f"Research source digest mismatch: {target}")
            else:
                temporary = target.with_suffix(f".tmp.{os.getpid()}")
                temporary.write_bytes(source.read_bytes())
                temporary.replace(target)
            kernel["source"]["path"] = str(target.relative_to(opts.output_dir))
            runtime._load_source(target, digest, kernel["arch"], 0)
        return dict(
            kind="exported",
            id=opts.id,
            kernel=kernel,
